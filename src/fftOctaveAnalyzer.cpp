...
#define YSCALER		0.5f		// TODO: fix this scaler 
...
void FFTOctaveAnalyzer::calculate(float * fftData){
    ...
    for (int speidx=0; speidx < nSpectrum; speidx++) {
		...
		if (avgidx != last_avgidx) {
			...
			for (int j = last_avgidx; j < avgidx; j++){
				averages[j] = sum * YSCALER * audioMult / count;
				if(averages[j] > 1) averages[j] = 1;
			}
			count = 0;
			sum = 0.0f;
		}
    ...
    if ((count > 0) && (last_avgidx < nAverages)){
		averages[last_avgidx] = sum * YSCALER * audioMult / count;
		if(averages[last_avgidx] > 1) averages[last_avgidx] = 1;
	}
